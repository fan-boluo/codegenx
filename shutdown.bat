@echo off
rem Usage:
rem   shutdown.bat                    -> stop user-service, ai-service, app-service, api-gateway
rem   shutdown.bat user ai app        -> stop selected services only
...
if /I "%~1"=="user" (
    ...
    echo Usage: shutdown.bat [user] [ai] [app] [gateway]
    exit /b 1
)
