<template>
  <div id="profilePage">
    ...
    <a-row :gutter="16">
      ...
      <a-col :span="8">
        <a-card title="配额信息" :loading="loading">
          ...
          <a-statistic
            title="剩余配额"
            :value="quotaInfo.remainingQuota === -1 ? '无限制' : quotaInfo.remainingQuota"
          ...
          >
            <template #bodyCell="{ column, record }">
              <template v-if="column.dataIndex === 'amount'">
                ...
                <span v-else style="color: #cf1322; font-weight: bold">
                  -¥{{ record.amount?.toFixed(4) }}
                </span>
              </template>
              <template v-else-if="column.dataIndex === 'billingType'">
                <a-tag v-if="record.billingType === 'api_call'" color="orange"> API调用 </a-tag>
                <a-tag v-else-if="record.billingType === 'recharge'" color="green"> 充值 </a-tag>
    ...
    <!-- 充值弹窗 -->
    <a-modal v-model:open="rechargeModalVisible" title="账户充值" :footer="null" width="500px">
      <a-form :model="rechargeForm" layout="vertical" @finish="handleRecharge">
        ...
        <a-form-item>
          <a-space>
            <a-button type="primary" html-type="submit" :loading="recharging"> 立即支付 </a-button>
    ...
    tooltip: {
      ...
      formatter: (params: any) => {
        ...
        params.forEach((param: any) => {
          const value =
            param.seriesName === '费用（元）' ? `¥${param.value.toFixed(2)}` : param.value
