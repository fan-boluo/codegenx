...
class ChatService:
    ...
    async def chat_to_gen_code(
        ...
        log.info(
            "app-service codegen start traceId={} appId={} userId={} stream={} messageLen={} preview={}",
            trace_id,
            request.app_id,
            login_user.user_id,
            request.stream,
        ...
        try:
            log.info(
                "app-service calling ai-service traceId={} appId={} userId={} codeGenType={}",
                trace_id,
                request.app_id,
                login_user.user_id,
            ...
            async for chunk in self.ai_service_client.generate_code_stream(
                ...
                if monitor_context.first_chunk_latency_ms is None:
                    ...
                    log.info(
                        "app-service ai first chunk traceId={} appId={} latencyMs={} preview={}",
                        trace_id,
                        request.app_id,
            ...
            if ai_response_chunks:
                ...
                log.info(
                    "app-service ai completed traceId={} appId={} chunkCount={} resultLen={} preview={}",
                    trace_id,
                    request.app_id,
                ...
                save_req = GeneratedCodeSaveRequest(
                    appId=request.app_id,
                    codeGenType=code_gen_type.value if code_gen_type else None,
                    content=merged_code,
                )
                result = await AppService(self.db).save_generated_code(save_req)
                log.info(
                    "app-service save completed traceId={} appId={} result={}",
                    trace_id,
                    request.app_id,
                    result,
                )
            else:
                log.warning("app-service ai returned empty result traceId={} appId={}", trace_id, request.app_id)
        except Exception:
            log.exception(
                "app-service codegen failed traceId={} appId={} userId={} chunkCount={}",
                trace_id,
                request.app_id,
                login_user.user_id,
                monitor_context.chunk_count,
            ...
            log.info(
                "app-service codegen stream traceId={} appId={} userId={} upstreamInstance={} firstChunkLatencyMs={} totalLatencyMs={} chunkCount={}",
