...
async def chat_to_gen_code_get(
    ...
    log.info(
        "chat-service chat request traceId={} method=GET appId={} userId={} stream=true messageLen={} preview={}",
        trace_id,
        app_id,
...
async def chat_to_gen_code_post(
    ...
    trace_id = getattr(http_request.state, "trace_id", None)
    log.info(
        "chat-service chat request traceId={} method=POST appId={} userId={} stream={} messageLen={} preview={}",
        trace_id,
        payload.app_id,
        current_user.user_id,
        payload.stream,
    ...
    try:
        ...
        async for chunk in ChatService(db).chat_to_gen_code(payload, current_user, trace_id=trace_id):
            chunks.append(chunk)
        log.info(
            "chat-service non-stream completed traceId={} appId={} userId={} resultLen={} preview={}",
            trace_id,
            payload.app_id,
            current_user.user_id,
            len("".join(chunks)),
...
async def _stream_chat_response(http_request: Request, request: AppChatRequest, current_user: JWTUser, db: AsyncSession) -> StreamingResponse:
    ...
    async def event_stream():
        ...
        try:
            ...
            log.exception(
                "chat-service stream response failed traceId={} appId={} userId={}",
                trace_id,
                request.app_id,
                current_user.user_id,
            )
