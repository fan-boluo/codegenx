...
class VLLMClient:
    ...
    def __init__(self, base_url=None, model_name=None, api_key=None):
        ...
        self.api_key = api_key or provider_config.api_key or fallback_provider.api_key
        self.model_name = model_name or provider_config.model_name or default_agent.resolved_model_name
        self.model_base_url = base_url or provider_config.api_base or fallback_provider.api_base
        self.client = OpenAI(
            api_key=self.api_key,
            base_url=self.model_base_url
        )
        log.info(f"VLM Client {self.model_base_url}")
    ...
    def generate_caption(self, image_path: str) -> str:
        ...
        try:
            ...
            messages = VLLMClient().convert_messages_with_image_path(prompt, image_path)
            response = self.completions(messages, max_tokens=300, stream=False)
            return response
