...
class SessionManager:
    ...
    async def _rotate_files(self, session_id: str) -> None:
    ...
    async def save_history(self, session_id: str, history: list[dict[str, Any]],  user_id: str = "") -> None:
        """完整写入历史记录，超出 1MB 自动轮转到 _N.jsonl。"""
        await self._rotate_files(session_id)
    ...
    async def append_chat_history_message(self, session_id: str, message: dict[str, Any]) -> Path:
    ...
    async def append_tool_log(self, session_id: str, entry: dict[str, Any]) -> None:
    ...
    async def append_memory_log(self, session_id: str, entry: dict[str, Any]) -> None:
