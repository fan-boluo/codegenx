...
def apply_mmr(
    ...
    """
    最终得分 = 相关性权重 × 相关分数 - 多样性权重 × 与已选结果的最大相似度
    先选最相关的那个，原始得分最高的
        然后每次选：
        相关分数高
...
def normalize_scores(results: List[MemorySearchResult]) -> List[MemorySearchResult]:
    """
...
def merge_hybrid_results(
    ...
    log.debug(
        f"合并 {len(vector_results)} vector + {len(keyword_results)} keyword "
