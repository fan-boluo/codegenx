...
from app.core.constants import ErrorCode, API_KEY_STATUS_ACTIVE, INTERNAL_CHAT_IP_RATE_LIMIT_PER_SECOND
...
async def internal_chat_completions(
    ...
    if payload.stream:
        return StreamingResponse(
            service.chat_stream(payload, login_user.id, None, client_ip, None),
            media_type="text/event-stream",
        )
    response = await service.chat(payload, login_user.id, None, client_ip, None)
