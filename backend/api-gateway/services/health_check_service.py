...
"""
健康检查：
1)model_provider的健康检查，发起http异步请求，并更新健康状态、延迟、成功率
2)model的健康状态（根据request_log统计）、并更新健康状态、延迟、成功率、综合得分
3)根据model的统计结果，求平均，得到mode_provider的统计结果，并更新model_provider结果
"""
...
class HealthCheckService:
    def __init__(self, db: AsyncSession) -> None:
        ...
        # 历史检查结果，每个provider只存100个，滑动窗口记录
    ...
    async def check_provider_health(self, provider) -> bool:
        ...
        try:
            ...
            # 更新provider的检查结果 health_status \latency \ success_rate，作为后续综合评分依据
    ...
    async def sync_model_metrics_from_request_log(self) -> None:
        ...
        for model in active_models:
            ...
            health_status = self._determine_health_status(stats.success_rate)
    ...
    # 统计request_log表中每个模型的某段时间的延迟、成功率，并返回
    ...
    # 模型的统计指标，取出所有模型的指标，计算归一化所需最大最小值
    ...
    # 根据model的统计结果，求平均，得到mode_provider的统计结果，并更新model_provider结果
    async def _sync_provider_metrics_from_model_stats(self, models, stats_map: dict[int, _ModelStats]) -> None:
        ...
        for provider_id, model_list in grouped.items():
            ...
            health_status = self._determine_health_status(avg_success_rate)
            await self.model_provider_service.update_health_status(
                provider_id, health_status, avg_latency, avg_success_rate
            )

    # 对base_url发送了一个http异步请求
    @staticmethod
