...
from codegenx.ai_service.monitor.orm_models import MonitorAlertModel, SessionMetricsModel, SpanModel, TurnMetricsModel
from codegenx.ai_service.monitor.telemetry_schema import MonitorAlertRecord, SessionTelemetry, SpanRecord
        ...
        async def op(session: AsyncSession) -> None:
            ...
            await session.execute(
                mysql_insert(TurnMetricsModel),
                {
                    ...
                    "span_id": metrics.get("span_id") or "",
                    "turn_number": metrics["turn_number"],
                    "status": metrics["status"],
                    "end_reason": metrics.get("end_reason") or "",
