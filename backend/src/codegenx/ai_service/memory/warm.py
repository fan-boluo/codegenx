...
from codegenx.ai_service.memory.prompts import  FIND_RELEVANT_MD_USER_PROMPT, FIND_RELEVANT_MD_SYSTEM_PROMPT
...
def _keyword_overlap(query: str, description: str) -> float:
    ...
    Jaccard 相似度：交集数/并集数  因为query和description长度都不会太长
    """
...
async def find_relevant_topics(
    ...
    if len(candidates) > MAX_FILES_RETURNED:
        # 大于要求的个数，再用llm排
        # 第二步：LLM 精排
