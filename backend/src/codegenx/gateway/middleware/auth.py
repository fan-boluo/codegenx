...
async def login_and_create_token(response: Response, user) -> str:
    ...
    # Get current token_version for user-level mass revocation support
    ...
    token_version = await revoke_svc.get_user_token_version(user.id)
    token = JWTAuth.create_token(jwt_user, token_version=token_version)
